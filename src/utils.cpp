...
void initInputOutputPin( uint16_t GPIO_PORT, uint16_t GPIO_PIN )
{
	...
	RCC_AHB1PeriphClockCmd( GPIO_PORT_PERIPH_ADDR( GPIO_PORT ), ENABLE );
	GPIO_InitStruct.GPIO_Pin = GPIO_PIN;
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN | GPIO_Mode_OUT;
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_DOWN;
	GPIO_Init( GPIO_PORT_ADDR( GPIO_PORT ), &GPIO_InitStruct );
...
void    setPin( uint16_t GPIO_PORT, uint16_t GPIO_PIN, uint8_t value )
{
    if ( value )
    {
        ((GPIO_TypeDef *) (GPIO_PORT_ADDR(GPIO_PORT)))->BSRRH = 0x01 << GPIO_PIN;
    }
    else
    {
        ((GPIO_TypeDef *) (GPIO_PORT_ADDR(GPIO_PORT)))->BSRRL = 0x01 << GPIO_PIN;
    }
}
...
{
     return (((GPIO_TypeDef *) (GPIO_PORT_ADDR(GPIO_PORT)))->IDR & ( 0x1 << GPIO_PIN )) >> GPIO_PIN;
}
